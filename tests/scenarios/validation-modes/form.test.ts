...
describe('validation modes scenario', () => {
  it('validates on change, blur, all, and revalidation mode', async () => {
    const change = new BaseForm({ values: { name: '' }, mode: 'onChange' });
    ...
    const blur = new BaseForm({ values: { name: '' }, mode: 'onBlur' });
    ...
    const all = new BaseForm({ values: { name: '' }, mode: 'all' });
    ...
    const submitOnly = new BaseForm({ values: { name: '' }, mode: 'onSubmit', reValidateMode: 'onBlur' });
